# python scripts/data_preparation/PV_power_output.py

'''
This script identifies locations for which the PV data appears to be erroneous. They are identified by first calculating the total yearly energy production of all locations, and then looking for outliers.
...
The results are saved at: results/PV_ouput/
'''
...
# Define path to the results folder. Create one if it doesn't already exist
...
z_score_filter = 4 # This has been tuned by trial and error

for country in europe_grid.country.unique():
    # Skip the processing if a previous result already exists
    if os.path.isfile(os.path.join(results_path,f'{country}_PV.parquet.gzip')):
    ...
    pv_data = pd.read_parquet(os.path.join(SAF_directory,f'data/PVGIS/{country}_PV.parquet.gzip'))
    warnings.simplefilter("default", UserWarning)
    
    # Sum data over the whole year (Wh PV output per year) for each point and merge with the Europe grid geodataframe to assign data to evaluation points
    ...
    if len(outliers_df) == 0:
        ...
        pv_data.to_parquet(os.path.join(results_path,f'{country}_PV.parquet.gzip'),compression='gzip')
    else:
        # If outliers are found, they must be assigned imputed data
        ...
        pv_data.set_index(['lat','lon','time']).to_parquet(os.path.join(results_path,f'{country}_PV.parquet.gzip'),compression='gzip')
