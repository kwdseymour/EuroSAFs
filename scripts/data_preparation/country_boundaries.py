# python scripts/data_preparation/country_boundaries.py
'''
...
--- EuroSAFs/data/Countries_WGS84/processed/Europe_WGS84.shp

In the second part of the notebook, the evaluation grid and evalution points are determined:  
The resolution of hourly wind & PV data is limited by the MERRA wind data (0.5 x 0.625°). 
This is the geospatial resolution used for all analysis in this notebook and beyond. 
Thus, each country is divided into a grid with the MERRA resolution and each cell is represented by the lat & lon coordinates from the MERRA resolution on which the cell is centered ("grid_lat" & "grid_lon"). 
For cells along shorelines and borders, the cells are not rectangular and the "grid_lat" & "grid_lon" point representing the cell may not fall within the cell boundaries (geometry). 
Thus, for each cell, the centroid of the largest polygon of the geometry is determined ("cent_lat" & "cent_lon"). 
These points are used to query the PVGIS API for PV data at that point in the PVGIS_download.py script (an attempt to query a point that is over the sea/ocean will return an error).

The set of gridded countries is saved as a shapefile to:  
--- EuroSAFs/data/Countries_WGS84/processed/Europe_Evaluation_Grid.shp*

The set of grid points (evaluation points) for each country is saved as a shapefile to:  
--- EuroSAFs/data/Countries_WGS84/processed/Europe_Evaluation_Points.shp 
and as a JSON file to:  
--- EuroSAFs/data/Countries_WGS84/processed/Europe_Evaluation_Points.json
and as a CSV file to:  
--- EuroSAFs/data/Countries_WGS84/processed/Europe_Evaluation_Points.csv
___
'''
...
results_path = os.path.join(SAF_directory,'data/Countries_WGS84/processed')
...
# Import world country borders shapefile
...
for country in europe_unfiltered.country.unique():
    ...
    # Only perform this filtering step if a country is a multipolygon
    if type( europe_unfiltered.loc[europe_unfiltered.country==country,'geometry'].item()) == shapely.geometry.multipolygon.MultiPolygon:
        ...
        # Convert multipolygon to a new GeoDataFrame (explode)
        poly_df = gpd.GeoDataFrame({'geometry':list(multipolygon)})
        poly_df.crs = world.crs
        poly_df.to_crs('epsg:3035',inplace=True) # EPSG 3035 is a CRS for equal area in Europe
        ...
        # Calculate the distance from each polygon to the biggest polygon ("mainland")
        for idx in poly_df.index:
            poly_df[f'distance_to_main'] = poly_df.distance(poly_df.at[poly_df.area_sqkm.idxmax(),'geometry'])/1e3 # [km]

        '''Create a mask of the rows to be removed based on the following conditions:
            - The land area is less than 20 km2
                OR
            - The distance to the biggest geometry ("mainland") is greater than 4 times the length of the given geometry (eliminates distant territories)
        '''
        removal_mask = (poly_df.area_sqkm < 20) | (poly_df.distance_to_main > poly_df.length_km * 4)
        removed_polygons[country] = np.sum(removal_mask)
        poly_df = poly_df.loc[~removal_mask]

        # Recombine the exploded GeoDataFrame to a single geometry in a new GeoDataFrame
...
europe.to_file(os.path.join(results_path,'Europe_WGS84.shp'))

# Create a geometry representing all the land mass in Europe. 
# Slight buffering is necessary to overcome geometry imprecisions along borders that would otherwise produce holes
...
def generated_gridded_country(country, map_eu):
    ...
    # Buffer the country geometry by 100 km and find the bounds of the resulting geometry
    # The buffering is done to inclued locations up to 100 km offshore of the country
    ...
    if country=='Spain': # This is a manual fix to a processing anomaly that would otherwise cause downstream errors
        cent_lat += 1

    # Generate a new CRS projection in order to create an accurate buffer of 100km. 
    ...
    country_geom_aeqd = map_eu.loc[map_eu.country==country].copy().to_crs(crs=aeqd) # Reproject to newly created CRS
    buffered_country = country_geom_aeqd.buffer(100000).to_crs(map_eu.crs) # Create a geometry that is buffered 100km around the original country geometry and reproject to original CRS

    # Get the bounds of the buffered country and initialize x's and y's according to the MERRA resolution to generate a grid
    ...
    # Create a new GeoDataFrame from the grid points
    ...
    # Find the intersection of the grid with the buffered country geometry (up to 100 km offshore)
    ...
    # The following bounding box gdf limits the extents of the world buffer. This avoids a problem with reprojection
    ...
    # Extract the gridded geometries that are in the sea
    sea_grid = gpd.overlay(sea_grid,buffered_world.to_crs(map_eu.crs),how='difference')

    # Identify points that are within the minimum offshore distance
...
def assign_merra_points(country_df_raw):
    ...
    For offshore nodes in which the nominal MERRA point resides on land, the next closest offshore MERRA point it assigned instead.
    For onshore nodes in which the nominal MERRA point resides offshore, the next closest onshore MERRA point is assigned instead.
    ...
    # Generate a new CRS projection in order to calculate accurate distances. 
    # The following line is adapted from here: https://automating-gis-processes.github.io/site/2018/notebooks/L2/projections.html
   ...
   # Duplicate the dataframe and convert each grid cell's centroid to the geometry column
    ...
    centroids.to_crs(aeqd,inplace=True) # Reproject to CRS for accurate distance caluclations
    ...
    grid_points.to_crs(aeqd,inplace=True) # Reproject to CRS for accurate distance caluclations
...
def get_offshore_distance(country_df_raw,europe_df,country):
    ...
    country_df['shore_dist'] = points_df_aeqd.distance(country_geom_aeqd)/1e3 #[km]
...
# Assign centroid coordinates as the pPV coordinates to be used to query PV data in the PVGIS_download.py script
europe_grid['pv_lat'] = europe_grid['cent_lat']
europe_grid['pv_lon'] = europe_grid['cent_lon']
# If the point is offshore, assign NaN
...
europe_grid.to_file(os.path.join(results_path,'Europe_Evaluation_Grid.shp'))

# Onshore & offshore evaluation grids
europe_grid.loc[~europe_grid.sea_node].reset_index(drop=True).to_file(os.path.join(results_path,'Onshore_Evaluation_Grid.shp'))
europe_grid.loc[europe_grid.sea_node].reset_index(drop=True).to_file(os.path.join(results_path,'Offshore_Evaluation_Grid.shp'))

# Combined onshore & offshore evaluation points (also save to CSV)
...
europe_points.to_file(os.path.join(results_path,'Europe_Evaluation_Points.shp'))
europe_points.to_csv(os.path.join(results_path,'Europe_Evaluation_Points.csv'))

# Combined onshore & offshore PV evaluation points
...
europe_PV_points.to_file(os.path.join(results_path,'Europe_PV_Evaluation_Points.shp'))

# Combined onshore & offshore MERRA wind evaluation points
...
europe_merra_points.to_file(os.path.join(results_path,'Europe_MERRA_Evaluation_Points.shp'))
...
with open(os.path.join(results_path,'Europe_Evaluation_Points.json'), 'w') as fp:
    json.dump(europe_points_dict, fp)

with open(os.path.join(results_path,'Europe_PV_Evaluation_Points.json'), 'w') as fp:
    json.dump(pv_points_dict, fp)
    
with open(os.path.join(results_path,'Europe_MERRA_Evaluation_Points.json'), 'w') as fp:
